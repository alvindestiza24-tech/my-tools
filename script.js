...
function renderInstagram(body) {
    ...
    document.getElementById('instaBtn').onclick = async () => {
        let url = document.getElementById('instaUrl').value.trim();
        const target = document.getElementById('instaResult');

        if (!url) return alert('Masukkan link Instagram!');

        // --- PEMBERSIH LINK AUTOMATIS ---
        // Mengubah Https:// jadi https://
        url = url.replace(/^Https:/i, 'https:');
        
        // Memotong buntut tracking ?igsh=...
        if (url.includes('?')) {
            url = url.split('?')[0];
        }
        ...
        try {
            ...
            if (dl) {
                ...
                target.innerHTML = `<div class="result-box" style="color:#ef4444;">Gagal mengambil media. Pastikan akun tidak di-private!</div>`;
            }
        } catch (e) {
