...
function renderInstagram(body) {
    ...
    document.getElementById('instaBtn').onclick = async () => {
        ...
        try {
            ...
            // RapidAPI membutuhkan headers ini agar request tidak ditolak
            const options = {
                method: 'GET',
                headers: {
                    'x-rapidapi-key': 'bd42c4bb5emsh944ed1ec30cb158p1b5755jsn0f7c4f7092a6',
            ...
            // Memeriksa beberapa kemungkinan properti URL dari hasil JSON
            let dl = data.result?.[0]?.url || data.result?.url || data.url || data.download_url;
