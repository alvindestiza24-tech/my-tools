...
function renderInstagram(body) {
    ...
    document.getElementById('instaBtn').onclick = async () => {
        ...
        try {
            // Menggunakan API publik alternatif yang mendukung panggilan langsung dari browser
            const apiUrl = `https://api.vreden.web.id/api/instagram?url=${encodeURIComponent(url)}`;
            
            const res = await fetch(apiUrl);
            const data = await res.json();

            // Cek struktur hasil respon dari API
            let dl = data.result?.url?.[0] || data.result?.url || data.result?.media?.[0] || data.result?.[0]?.url;
                ...
                target.innerHTML = `
                    <div class="result-box">
                        ...
                        <a href="${dl}" target="_blank" download style="text-decoration:none;">
                            <button class="v-btn" style="background:rgba(168,85,247,0.12);">Download Video</button>
                        </a>
                    </div>
                `;
            } else {
                target.innerHTML = `<div class="result-box" style="color:#ef4444;">Gagal mengambil media. Pastikan akun tidak di-private atau coba link lain!</div>`;
            }
        } catch (e) {
            console.error(e);
...
function renderWinquotes(body) {
    body.innerHTML = `
        ...
        <div class="result-box" style="padding:30px;">
            ...
            <p style="color:#b0a0d0;font-size:13px;">Fitur Windows Quotes sedang dalam perbaikan akibat spam API. Silakan tunggu update selanjutnya.</p>
            <a href="https://whatsapp.com/channel/0029VbDJSXIKAwEqsJbhgA0s" target="_blank" class="v-btn" style="background:rgba(37,211,102,0.15);text-decoration:none;display:inline-block;margin-top:12px;width:auto;padding:10px 20px;">
...
function renderTanyaUstadz(body) {
    body.innerHTML = `
        ...
        <div class="result-box" style="padding:30px;">
            <i class="fas fa-tools" style="font-size:50px;color:#6a5a8a;margin-bottom:15px;display:block;"></i>
            <h3 style="color:#c084fc;margin-bottom:10px;">⛔ Fitur Mati Sementara</h3>
            <p style="color:#b0a0d0;font-size:13px;">Fitur Tanya Ustadz sedang dalam perbaikan akibat spam API. Silakan tunggu update selanjutnya.</p>
            <a href="https://whatsapp.com/channel/0029VbDJSXIKAwEqsJbhgA0s" target="_blank" class="v-btn" style="background:rgba(37,211,102,0.15);text-decoration:none;display:inline-block;margin-top:12px;width:auto;padding:10px 20px;">
                <i class="fab fa-whatsapp"></i> Ikuti Saluran Admin
            </a>
        </div>
